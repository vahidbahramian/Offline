...
namespace NS_DSP
{
	...
	BOOL CHFAnalogAMRvmld::Initialize(int nFftSize)
	{
		...
		if((ret = m_Hilbert.Create(8000))!= DSP_RET_SUCCESS)
			return ret;
	...
	int CHFAnalogAMRvmld::CarrierFreqIndex(double *m_fSamples,int m_nNumberSamples)
	{
		int i;
		MKL_LONG Status;
		...
		int m_iMaxIndex = cblas_idamax(m_nFftSize/2,m_fFFTAbsSum,1);
		return m_iMaxIndex;
	}
	...
	BOOL CHFAnalogAMRvmld::DoAMR(double *m_fSamples,double fs,int m_nNumberSamples,
		BOOL *m_bArrValidChunk,int m_nBlocks,int &modType)
	{
		int i;
		double line;
		...
		res = m_Hilbert.Filter(m_fSamples,m_nNumberSamples,m_cHilbertOut,m_iNumHilbertSamples);
		...
		int nSamples = floor(double(fs/100.0)) > 8192 ? 8192 : floor(double(fs/100.0));
		int blk;
		int nNumberR = floor(double(8192)/double(nSamples));
		double *m_fRArray = new double[nNumberR * m_nBlocks];
		int nRCntr=0;
		if(!m_fRArray) return FALSE;
		for (blk=0;blk<m_nBlocks;blk++)
		{
			if (m_bArrValidChunk[blk]==FALSE) continue;
			for (i=0;i<nNumberR;i++)
			{
				vdPowx(nSamples,m_fEnvelope+i*nSamples,2,m_fEnvPow2+i*nSamples);
				if((res= vmldVar(m_fEnvPow2+i*nSamples,nSamples,m_fRNum))!=DSP_RET_SUCCESS)
					return res;
				if((res =vmldMean(m_fEnvPow2+i*nSamples,nSamples,m_fRDen))!=DSP_RET_SUCCESS)
					return res;
				m_fRArray[nRCntr++] = m_fRNum / (m_fRDen*m_fRDen);//	double m_fR = var (env.^2) / ((mean(env.^2))^2);
			}
		}//for blk

		int nInvalidRCount=0;
		int nValidRCount=0;
		for (i=0;i<nRCntr;i++)
		{
			if (m_fRArray[i] > 2.1) nInvalidRCount++;
			else m_fRArray[nValidRCount++] = m_fRArray[i];
		}
		if (double(nInvalidRCount)/double(nRCntr) >0.5)
		{
		...
		if((res = vmldMean(m_fRArray,nRCntr,m_fR))!=DSP_RET_SUCCESS)
			return res;
		...
		afxDump << "AMR Result = " ;
		afxDump << "\tfc = " << fc;
		afxDump << "\tm_fSigmaDP = " <<m_fSigmaDP;
		afxDump << "\tm_fMeanGamma = " <<m_fMeanGamma;
		afxDump << "\tm_fR = " <<m_fR;
		afxDump << "\tP = " <<P;
		afxDump << "\n";
#endif
...
//power estimation for validity of input signal
BOOL IsValidAMRSignal(double *m_fSamples,int m_nSize ,double m_fThreshold,
					  BOOL *bArrValidChunk,int &nInvalidCntr,int &result)
{
	//AR filter for smoothing :	y(n) = (1-b)*y(n-1) + b*x(n)^2
	double ynD1=0,yn=0;//y(n-1),y(n)
	int i;
	nInvalidCntr=0;
	result = AMR_POWER_COMPARISON_INVALID;
	int ret;
	...
	//test
	//FILE *fpt = fopen("C:\\samples\\MA.txt","wt");
	//for(i=0;i<m_nSize;i++) fprintf(fpt,"%f\n",m_fSamples[i]);
	//fclose(fpt);
	//test
...
#endif
	//check for validity
	result = AMR_POWER_COMPARISON_VALID;
	//	for(i=0;i<blk;i++)
	//		if(m_fSamples[i] < m_fThreshold) result = FALSE;//1e-5

	ms=0;
	for(i=0;i<blk;i++)
		if (ms<m_fSamples[i]) ms = m_fSamples[i];
	nInvalidCntr=0;
	for(i=0;i<blk;i++){
		if(m_fSamples[i] < 0.05*ms ) 
		{
			nInvalidCntr++;
			bArrValidChunk[i] = FALSE;
		}else{
			bArrValidChunk[i] = TRUE;
		}
	}//for
	if (nInvalidCntr ==0)
	{
		...
		if (nInvalidCntr >5)  result = AMR_POWER_COMPARISON_INVALID;
		else   result = AMR_POWER_COMPARISON_HIGH_VARIATION;
	}
	delete del1024;
	return DSP_RET_SUCCESS;
}
